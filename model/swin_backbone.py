""" Swin Transformer — adapted for FoodMamba V9
https://arxiv.org/pdf/2103.14030
Weights from https://github.com/microsoft/Swin-Transformer
"""
...
def swin_base_patch4_window7_224_in22k(num_classes=21841, **kwargs):
    model = SwinTransformer(in_chans=3, patch_size=4, window_size=7,
                            embed_dim=128, depths=(2, 2, 18, 2),
                            num_heads=(4, 8, 16, 32), num_classes=num_classes, **kwargs)
    return model


def swin_base_patch4_window12_384_in22k(num_classes=21841, **kwargs):
    model = SwinTransformer(in_chans=3, patch_size=4, window_size=12,
                            embed_dim=128, depths=(2, 2, 18, 2),
                            num_heads=(4, 8, 16, 32), num_classes=num_classes, **kwargs)
    return model
...
def load_swin_weights(model, weights_path):
    ...
    if missing:
        print(f"Swin weights missing keys: {len(missing)} (expected for no head)")
    if unexpected:
        print(f"Swin weights unexpected keys: {len(unexpected)}")
    return model
